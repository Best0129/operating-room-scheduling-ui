...
with st.sidebar:
    ...
    num_generations = col_p1.number_input("Generations", 10, 1000, value=100, disabled=is_st_selected)
    pop_size = col_p2.number_input("Population", 10, 500, value=100, disabled=is_st_selected)
    
    # ดึงน้ำหนักจาก Config ของแต่ละโหมด
    cfg = CONFIGS[exp_mode]
    st.caption(f"Weights Config: Makespan({W_MAKESPAN}) | Overtime({W_OVERTIME}) | Balance({W_IMBALANCE})")
    run_button = st.button("เริ่มการประมวลผล", use_container_width=True)
...
if st.session_state.results:
        ...
        for day in sorted(res['sched'].keys()):
            with st.expander(f"🗓️ วันที่ {day + 1}", expanded=(day==0)):
                for or_id in sorted(res['sched'][day].keys(), key=lambda x: str(x)):
                    ...
                    cases = pd.DataFrame([{
                        "ID": c['Encounter ID'], "แผนก": c['Service'],
                        "เริ่ม": slot_to_time(c['start_slot'], res['op_time'], res['slot_dur']),
                        "จบ": slot_to_time(c['end_slot'], res['op_time'], res['slot_dur']),
                        "น้ำหนัก": c['Weight']
    ...
    with tab_graph:
        if len(res['history']) > 1:
            ...
            st.info("💡 Heuristic คำนวณรอบเดียวไม่มีกราฟ")
